...
{
    public partial class UcChangelogPage : UserControl, IUserControlMisc
    {
        private int _viewPos, _viewCount = 34;

        public int DisplayIndex { get; set; }

        public UcChangelogPage()
        {
            InitializeComponent();
        ...
        public void Loaded()
        {
            ...
            Data.OnUpdateWorkLogFilter += Data_OnUpdateChangelogFilter;
            Data.OnUpdateChangeLog += Data_OnUpdateChangelog;
        }

        void Data_OnUpdateChangelog(object sender, EventArgs e)
        {
            if (!tsbEnd.Enabled)
                SelectEndRecords();
        }
        ...
        private static string FineAddr(string overpass, string way, string product, string riser)
        {
            var rec = string.Concat(overpass, way, product, riser);
            if (string.IsNullOrWhiteSpace(rec))
                return "";
            if (string.IsNullOrWhiteSpace(riser))
                return string.Format("Эстакада {0}. Путь {1}. {2}", overpass, way, product);
            return string.Format("Эстакада {0}. Путь {1}. {2}. Стояк {3}",
        ...
        void Data_OnUpdateChangelogFilter(object sender, EventArgs e)
        {
            SelectEndRecords();
        }
